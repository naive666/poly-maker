...
def process_book_data(asset, json_data):
    global_state.orderbook_data[asset] = {
        'asset_id': json_data['asset_id'],  # token_id for the Yes token
        'bids': SortedDict(),
        'asks': SortedDict()
    }

    global_state.orderbook_data[asset]['bids'].update({float(entry['price']): float(entry['size']) for entry in json_data['bids']})
    global_state.orderbook_data[asset]['asks'].update({float(entry['price']): float(entry['size']) for entry in json_data['asks']})

def process_price_change(asset, side, price_level, new_size):
    if asset_id != global_state.orderbook_data[asset]['asset_id']:
        return  # skip updates for the No token to prevent duplicated updates
    if side == 'bids':
        book = global_state.orderbook_data[asset]['bids']
    else:
        book = global_state.orderbook_data[asset]['asks']
...
def process_data(json_datas, trade=True):
    ...
    for json_data in json_datas:
        ...
        # pretty_print(f'Received book update for {asset}:', global_state.orderbook_data[asset])
